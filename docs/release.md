...
checksums. Two fresh, channel-specific jobs in the protected `release`
environment then recheck the tag, clean tracked state, exact untracked bundle,
and all bundle digests. The GitHub job publishes the verified assets and cannot
receive the tap token. Only after that succeeds can the Homebrew job run; it
has read-only repository permissions and receives the
...
Release artifacts are intentionally not attested or independently signed at
this time. Depot CI issues OIDC identity from `identity.depot.dev`, which is not
accepted by GitHub's native artifact-attestation signing path. The incompatible
GitHub action was removed rather than weakening verification, moving releases
to another CI backend, or introducing a long-lived signing key without a key
management and verifier policy. This supersedes the earlier CLI-022 attestation
decision. Revisit when a Depot-compatible keyless signing mechanism has a
documented trust root and pinned consumer-side verifier. Until then, checksums
detect corruption only when obtained from a trusted GitHub Release; they do not
independently prove CI provenance. Digests do not make adversary code safe to
execute; see the trust model.
...
out and verify the immutable tag, download and verify the exact built bundle,
