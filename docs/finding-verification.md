...
A high-confidence, source-cited **keep** enters the normal composition merger. A supported **reject** stays in the decision ledger and original replay pool. **Unresolved** candidates receive one bounded retrieval-and-recheck pass when the verifier requests more source. They remain visible in the review observations if they cannot be established; they are not posted as confirmed findings. Unresolved decisions do **not** fail the review or discard verified peers. The normal finding exit code applies when verified findings remain; otherwise the review succeeds with an empty finding list. A clean/ship opinion is withheld whenever any candidate remains unresolved. Per-candidate provider or schema failures also produce explicit unresolved decisions rather than bypassing verification. Cancellation, invalid report/snapshot structure, failed artifact writes and reviewer execution failures remain errors.
