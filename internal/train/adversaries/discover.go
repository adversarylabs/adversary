...
// DiscoverCatalogRoot loads private catalog adversaries for evidence routing.
// Their README remains the routing mission even when the package also contains
// the executable model-backed runtime generated by `adversary catalog init`.
func DiscoverCatalogRoot(root string) ([]Package, error) {
	abs, err := filepath.Abs(root)
	if err != nil {
		return nil, err
	}
	...
	sort.Slice(out, func(i, j int) bool { return out[i].ID < out[j].ID })
	if len(out) == 0 {
		return nil, fmt.Errorf("no catalog adversaries with README.md under %s", abs)
	}
	return out, nil
}
