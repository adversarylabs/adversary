...
func TestNonActionableReplyRejectsNitStatusOnly(t *testing.T) {
	for _, body := range []string{
		"Nit: fixed, thanks!",
		"Nit: fixed by naming the request ID in the error.",
	} {
		reason, rejected := NonActionableReply(body)
		if !rejected || !strings.Contains(reason, "status reply") {
			t.Fatalf("nit status reply was not rejected: body=%q rejected=%v reason=%q", body, rejected, reason)
		}
	}
}

func TestNonActionableReplyKeepsSubstantiveNit(t *testing.T) {
	body := "Nit: rename this to requestID so the error names the failing request."
	if reason, rejected := NonActionableReply(body); rejected {
		t.Fatalf("substantive nit was rejected as %q", reason)
	}
}

func TestIsAutomatedReviewerUsesGitHubProvenance(t *testing.T) {
	for _, tc := range []struct {
		name, login, userType, appSlug string
		automated                      bool
	}{
		{name: "bot actor", login: "review-service", userType: "Bot", automated: true},
		{name: "github app", login: "human-looking", userType: "User", appSlug: "ai-reviewer", automated: true},
		{name: "legacy login", login: "coderabbitai[bot]", automated: true},
		{name: "human", login: "maintainer", userType: "User", automated: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := IsAutomatedReviewer(tc.login, tc.userType, tc.appSlug); got != tc.automated {
				t.Fatalf("IsAutomatedReviewer() = %v, want %v", got, tc.automated)
			}
		})
	}
}

func TestAutomatedReviewArtifactRecognizesExplicitAIMarker(t *testing.T) {
	for _, body := range []string{
		"This callback can race with shutdown.\n\n<!-- generated by CodeRabbit -->",
		"Potential issue.\n\n<!-- This is an auto-generated comment: review status by coderabbit.ai -->",
	} {
		if !IsAutomatedReviewArtifact(body) {
			t.Fatalf("explicit AI generation marker was not recognized: %q", body)
		}
	}
}
