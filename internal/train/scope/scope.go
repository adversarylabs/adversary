...
// treating a substantive nit or follow-up question as resolved. In particular,
// the "nit:" label is normally a review request, but in "nit: fixed" it merely
// describes the concern that was addressed.
func isStatusOnlyReply(lower string) bool {
	if lower == "" || len([]rune(lower)) > 100 {
		return false
	}
	trimmed := strings.TrimSpace(lower)
	for _, prefix := range []string{"nit:", "nit;", "nit -", "**nit:**"} {
		if rest, found := strings.CutPrefix(trimmed, prefix); found && isAuthorStatusUpdate(strings.TrimSpace(rest)) {
			return true
		}
	}
	...
	allowed := map[string]bool{
		"nit": true, "fixed": true, "done": true, "addressed": true,
		"resolved": true, "updated": true, "applied": true, "implemented": true,
		"thanks": true, "thank": true, "you": true, "agreed": true,
		"yes": true, "yep": true, "right": true, "good": true, "catch": true,
		"now": true, "also": true,
	}
	hasStatus := false
	for _, word := range words {
		...
		switch word {
		case "nit", "fixed", "done", "addressed", "resolved", "updated", "applied", "implemented":
			hasStatus = true
		}
	}
	return hasStatus
...
// IsAutomatedReviewArtifact reports whether review prose carries provenance
// from a known automated reviewer, even when GitHub attributes it to a human
// account. Collection uses this to keep inline children from becoming gold
// when the marker exists only on their parent review body.
func IsAutomatedReviewArtifact(body string) bool {
	lower := strings.ToLower(body)
	markers := []string{
		"<!-- hermes-pr-review ",
...
// IsAutomatedReviewer reports high-confidence GitHub provenance that cannot be
// inferred reliably from prose alone. GitHub's Bot actor type and a non-empty
// performed_via_github_app slug both mean the review was emitted by software;
// login matching remains a fallback for older cached API payloads.
func IsAutomatedReviewer(login, userType, appSlug string) bool {
	if strings.EqualFold(strings.TrimSpace(userType), "bot") || strings.TrimSpace(appSlug) != "" {
		return true
	}
	return isReviewBot(strings.ToLower(strings.TrimSpace(login)))
}
