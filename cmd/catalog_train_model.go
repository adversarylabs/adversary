...
The implementation must inspect concrete repository facts without calling ctx.model. For Go rules involving types, binding scope, calls, assignments, containment, or operation order, you MUST use ctx.repoGraph?.semanticMatches with a declarative SemanticQuery. The CLI owns parsing and type resolution; never implement a tokenizer, balanced-brace scanner, source parser, or regular-expression approximation of those facts inside the adversary. semanticMatches accepts { language: "go", within: "function", steps: [...] }. Each step has kind ("call" | "assignment" | "return" | "condition") and may capture itself. Call steps may constrain name, method, receiverType. Assignment steps may constrain within (a captured call), after (a captured operation), operator, sourceKind ("call" | "expression"), and targets such as { capture, scope: "package" | "parameter" | "local" | "field" | "unknown", type }. Return and condition steps may use references to require a previously captured binding. Each result has stable key, path, line, column, unit, and captures. Return safely with no findings when repoGraph is unavailable. Filter matches to ctx.listInScopePaths() before emitting findings. Use the rule id plus match.key as groupKey so results are stable and distinct.

Native tests for a semantic rule must inject a minimal RepoGraph test double through createApp().run({ repoGraph, ... }) and vary the semantic facts directly: one matching typed/scoped relationship, one close allowed relationship, and one shadowing or unrelated decoy. Do not retest language parsing in each adversary; the CLI's repository-index tests own parser correctness. Use ctx.loadInScopeSources only for genuinely textual rules that do not depend on type, binding scope, containment, or operation order. Its result is Array<{ path: string; content: string; status: "changed" | "repository" }>. Its only supported option fields are include: (path: string) => boolean, limit: number, ignoreDirectories: readonly string[], and maxBytes: number. Never invent options such as maxLines, maxFiles, maxTotalBytes, or maxBytesPerFile.

The implementation must establish the complete finding condition within the same relevant semantic unit. Do not combine independent facts without expressing the required containment, ordering, captured-binding, scope, and type relationships in the query. Every emitted summary and evidence message must be proven by query predicates. For genuinely textual non-semantic policies, keep any textual scan narrowly bounded and do not combine unrelated file-wide tokens.

Never emit a finding merely because the original evidence path is present or changed, never hard-code an evidence line number, and generalize beyond the original filename unless the accepted policy is intrinsically path-specific. Derive the positive semantic test double from the relationships demonstrated by evidence_diff plus evidence_source_context; do not replace it with an easier invented condition. Add at least three production-entry-point cases at the exact same repository-relative path: the evidence-grounded positive; a close allowed case differing only in policy-relevant semantic facts; and a shadowing or unrelated decoy that must produce no finding. Put each materially different case in its own named native test, or give every finding-count assertion a case-specific message, so compiler feedback identifies the exact positive or negative that failed. In tests, assert the finding count before reading findings[0] or its properties so a missing match produces an actionable assertion rather than a TypeError. Do not import or invoke the new rule helper directly from its test. Preserve all existing deterministic registrations in src/deterministic.ts. Do not emit rules/*/rule.yaml or cases.yaml for this strategy. During repair, retain the prior plan and make the smallest coherent correction that satisfies the critic; do not regenerate unrelated files or redesign already-accepted portions.
...
func reviewGeneratedDeterministicRule(ctx context.Context, provider application.ModelReviewProvider, request catalogapply.ChangeRequest, plan catalogapply.ChangePlan) error {
	...
	Return reject for a material soundness defect that must not be allowed through merely because the package compiles: a semantic Go rule does not use semanticMatches; the query omits a type, scope, containment, or order fact required by the emitted claim; the supplied positive is likely missed; an ordinary close case is likely flagged; or native tests do not inject matching, close-allowed, and shadowing or unrelated semantic results through createApp().run. Do not require each adversary to retest the CLI parser. YAML files under tests/ are descriptive evaluation evidence and are not executable. Return revise for bounded non-blocking improvements. Return accept when the declarative query establishes the material relationships, scope filtering and stable match identity are used, close semantic cases are rejected, and native tests exercise registration through createApp().run. If returning reject or revise, give the smallest finite correction grounded in these inputs; do not demand a custom parser, exhaustive language coverage, or access to code beyond the supplied evidence.`
	raw, err := provider.Review(ctx, application.ModelReviewRequest{
	...
	Return reject when an advertised prerequisite is absent from the semantic query, scope filtering is absent, stable match keys are not used, or the injected semantic test cases fail to distinguish a close allowed binding or relationship from the violation. Do not ask the adversary to parse source, and do not reject it for parser variants owned by the CLI semantic adapter. Return revise only for a non-blocking uncommon policy variant. Return accept if you can name the query constraints that establish each part of the emitted claim and the native tests cover the meaningful relationships. YAML evaluation cases are non-executable and do not satisfy native runtime-test requirements. Do not repeat the primary review or speculate. Reject only with an exact generated construct plus a concrete semantic result that produces the wrong outcome; otherwise accept or return a bounded revision.`
	raw, err = provider.Review(ctx, application.ModelReviewRequest{
