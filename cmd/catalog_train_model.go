...
Make the query exactly as general as the proposed policy. If the policy applies to any constructor or dependency type, capture an unconstrained call and an appropriately scoped target instead of naming the evidence's constructor or concrete dependency type; retain only policy-defining constraints such as the guard receiver type and the error result type. If the policy says an operation occurs inside a guard or callback, express that with within on the captured guard call. Pass the exact object returned by defineSemanticQuery directly to semanticMatches(query); do not rebuild a partially validated copy. State findings in terms of positive facts the query proves. Never claim that retry, reset, cleanup, authorization, or another safeguard is absent unless the query contains a predicate that proves that absence.
