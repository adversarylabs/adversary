...
If a positive test reports no finding, preserve its evidence-grounded SemanticMatch fixture and verify the test double returns it without interpreting the query. Trace scope filtering and finding mapping, then correct the first guard that rejects it. Do not weaken the assertion, change the positive fixture to fit the implementation, or emit an unconditional/path-only finding. If an empty-result test unexpectedly finds something, preserve that case and tighten only the predicate that admits it. Keep the query-contract, positive, and same-path empty-result cases intact after they have reached executable validation.
