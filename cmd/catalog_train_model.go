...
Use the smallest sequence that proves the positive data flow. When a binding is assigned inside a captured guard and then observed outside it, a return or condition step with after: "guardCapture" and references: "bindingCapture" directly proves that later observation. Do not insert an arbitrary "later accessor" call between them: repeated runtime entry into a function is not a second lexical call, and an unrelated intervening call adds no proof. Do not demand relationships the documented SemanticQuery cannot express; narrow the emitted finding to the positive relationships it can prove instead.
