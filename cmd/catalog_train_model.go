...
Make the query exactly as general as the proposed policy. If the policy applies to any constructor or dependency type, the factory call step MUST omit name and the dependency target MUST omit type: use an unnamed { kind: "call", capture: "factory", within: "guard" } and associate the assignment with source: "factory", while retaining the package scope and the policy-defining error trait. Never copy the evidence's constructor name or concrete dependency type into a general factory policy. If the policy says an operation occurs inside a guard or callback, express that with within on the captured guard call. Pass the exact object returned by defineSemanticQuery directly to semanticMatches(query); do not rebuild a partially validated copy. State findings in terms of positive facts the query proves. Never claim that retry, reset, cleanup, authorization, or another safeguard is absent unless the query contains a predicate that proves that absence.

Use the smallest sequence that proves the positive data flow. When a binding is assigned inside a captured guard and then observed outside it, a return or condition step with after: "guardCapture" and references: "bindingCapture" directly proves that later observation. When the same operation must observe several bindings, use references: ["firstBinding", "secondBinding"]. Do not insert an arbitrary "later accessor" call between them: repeated runtime entry into a function is not a second lexical call, and an unrelated intervening call adds no proof. Do not demand relationships the documented SemanticQuery cannot express; narrow the emitted finding to the positive relationships it can prove instead.
