...
For sync.Once fallible-result rules, reject titles or prose claiming the failure is permanent, retry is prevented, or reset/retry cases are excluded when the query only proves guarded assignment and later observation. Accept positive wording such as "Fallible factory result is cached by sync.Once" plus a retry/reset recommendation and an explicit note that separately implemented safeguards require reviewer judgment.`
	...
	For a sync.Once fallible-result query that has no negative safeguard predicate, reject "permanent", "prevents retries", and "does not report reset/retry" claims. Require positive structural-risk wording and allow retry/reset only as remediation or reviewer context.`
	raw, err = provider.Review(ctx, application.ModelReviewRequest{
