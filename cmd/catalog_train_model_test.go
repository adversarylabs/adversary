...
func TestCatalogChangePlannerUsesFocusedBuildAndTestRepairPrompt(t *testing.T) {
	...
	for _, want := range []string{"VALIDATION REPAIR MODE", "Do not weaken the assertion", "remove invented SDK options", "registered rule dispatch", "review-scope filtering", "semanticMatches-based design", "without interpreting the query", "repair the stale test assertion", "fixtureDirectory contains a real file", "non-empty array requiring the same operation", "joint observation", "exact direct right-hand-side call"} {
