...
func TestCatalogChangePlannerUsesFocusedBuildAndTestRepairPrompt(t *testing.T) {
	...
	for _, want := range []string{"VALIDATION REPAIR MODE", "first guard, extraction, or predicate", "Do not weaken the assertion", "remove invented SDK options", "balanced-brace scanning", "shadowing parameter", "holder.once.Do", "small Go lexical scanner", "must not parse declarations"} {
...
func TestCatalogChangePlannerKeepsRedTeamRejectionBlocking(t *testing.T) {
	provider := &catalogSequenceProviderStub{name: "camel", model: "auto", outputs: []json.RawMessage{
		...
		json.RawMessage(`{"disposition":"reject","reason":"The scanner accepts custom Do methods because it never verifies a sync.Once declaration."}`),
	}}
	runtime := &catalogModelRuntimeStub{provider: provider}
	previous := &catalogapply.ChangePlan{Strategy: catalogapply.StrategyDeterministic, Files: []catalogapply.ChangeFile{{Path: "adversaries/reliability/src/rules/lazy.ts", Content: "// second attempt"}}}
	_, err := catalogChangePlanner(runtime, "camel", "auto")(context.Background(), catalogapply.ChangeRequest{
	...
	if len(provider.requests) != 3 || !strings.Contains(provider.requests[2].Prompt, "A decoy split across separate functions is inadequate") || !strings.Contains(provider.requests[2].Prompt, "holder.once.Do") {
